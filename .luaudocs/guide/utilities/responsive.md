...
Where breakpoints answer "how big", `Flux.Responsive.platform`{lua} answers "what device": `"vr"`{luau}, `"console"`{luau} (ten-foot interface), `"pc"`{luau} (keyboard and mouse), or `"mobile"`{luau}. It is a plain string, not a node: the device class is detected **once at startup**, and off-client (server, Studio edit mode, headless) it is always `"pc"`{luau}:
