...
| Signature                                                                                         | Summary                                                   | Guide                                                                |
| :------------------------------------------------------------------------------------------------ | :-------------------------------------------------------- | :------------------------------------------------------------------- |
| `Flux.padding(value) -> UIPadding`                                                                | UIPadding from a length, per-side table, or reactive source. | [Layout](/guide/utilities/layout#padding)                         |
| `Flux.corner(radius?) -> UICorner`                                                                | UICorner from a radius (px or UDim).                      | [Layout](/guide/utilities/layout#corner)                             |
| `Flux.stroke(config?) -> UIStroke`                                                                | UIStroke (thickness, color, mode, joins).                 | [Layout](/guide/utilities/layout#stroke)                             |
| `Flux.list(config?) -> UIListLayout`                                                              | UIListLayout (gap, direction, align/justify flex semantics). | [Layout](/guide/utilities/layout#list)                            |
...
| `Flux.flex(mode?) -> UIFlexItem`                                                                  | UIFlexItem (`fill` default/`grow`/`shrink`/`none`).       | [Layout](/guide/utilities/layout#flex)                               |
...
| `Flux.Responsive.breakpoint: Node<Breakpoint>`                                                    | Size-class node derived from viewport width.              | [Responsive](/guide/utilities/responsive#breakpoint)                 |
| `Flux.Responsive.platform: Platform`                                                              | Startup device class: `"console"`/`"mobile"`/`"pc"`/`"vr"`. | [Responsive](/guide/utilities/responsive#platform)                 |
| `Flux.Responsive.scaleOf(partial?) -> Node<number>` · `breakpointOf(partial?) -> Node`            | Per-surface nodes; `viewport` source override = container queries. | [Responsive](/guide/utilities/responsive#container-queries-plugin-docks) |
| `Flux.Responsive.config`                                                                          | Mutable live default responsive config.                   | [Responsive](/guide/utilities/responsive#configuration)              |
| `Flux.exclusive() -> Group` · `group:activate/deactivate/toggle(state)` · `group:clear()`         | At most one active boolean node per group (menus, hover). | [Interact](/guide/utilities/interact#exclusive-groups)               |
| `Flux.pointIn(x, y, object) -> boolean`                                                           | Point-in-absolute-bounds test for a GuiObject.            | [Interact](/guide/utilities/interact#pointin)                        |
| `Flux.sunk(x, y, object) -> boolean`                                                              | Whether an Active gui above `object` claims the point.    | [Interact](/guide/utilities/interact#sunk)                           |
